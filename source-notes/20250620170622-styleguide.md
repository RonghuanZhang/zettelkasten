...
- [Test double packages](https://google.github.io/styleguide/go/#naming-doubles)
...
- When creating an error value, decide whether to give it any [structure](https://google.github.io/styleguide/go/#error-structure).
- When handling an error, consider [adding information](https://google.github.io/styleguide/go/#error-extra-info) that you have but that the caller and/or callee might not.
- See also guidance on [error logging](https://google.github.io/styleguide/go/#error-logging).
...
The same concept applies to [structured errors](https://google.github.io/styleguide/go/#error-structure) like [`*status.Status`](https://pkg.go.dev/google.golang.org/grpc/status) (see [canonical codes](https://pkg.go.dev/google.golang.org/grpc/codes)). For example, if your server sends malformed requests to a backend and receives an `InvalidArgument` code, this code should *not* be propagated to the client, assuming that the client has done nothing wrong. Instead, return an `Internal` canonical code to the client.
...
- [Error Documentation Conventions](https://google.github.io/styleguide/go/#documentation-conventions-errors)
...
- Avoid duplication. If you return an error, it’s usually better not to log it yourself but rather let the caller handle it. The caller can choose to log the error, or perhaps rate-limit logging using [`rate.Sometimes`](https://pkg.go.dev/golang.org/x/time/rate#Sometimes). Other options include attempting recovery or even [stopping the program](https://google.github.io/styleguide/go/#checks-and-panics). In any case, giving the caller control helps avoid logspam.
...
[Do not call `log` functions before flags have been parsed.](https://pkg.go.dev/github.com/golang/glog#pkg-overview)If you must die in a package initialization function (an `init` or a [“must” function](https://google.github.io/styleguide/go/decisions#must-functions)), a panic is acceptable in place of the fatal logging call.
...
Thoughtful application of these approaches can add [extra information to errors](https://google.github.io/styleguide/go/#error-extra-info) without much effort and help callers avoid adding redundant annotations.
...
> 
> For map and slice types, if the code is particularly performance sensitive and if you know the sizes in advance, see the [size hints](https://google.github.io/styleguide/go/#vardeclsize) section.
...
In contrast, using composite literals to declare empty or memberless values can be visually noisy compared to [zero-value initialization](https://google.github.io/styleguide/go/#vardeclzero).
...
**Warning:** Preallocating more memory than you need can waste memory in the fleet or even harm performance. When in doubt, see [GoTip #3: Benchmarking Go Code](https://google.github.io/styleguide/go/index.html#gotip) and default to a [zero initialization](https://google.github.io/styleguide/go/#vardeclzero) or a [composite literal declaration](https://google.github.io/styleguide/go/#vardeclcomposite).
...
Where a function requires many inputs, consider introducing an [option struct](https://google.github.io/styleguide/go/#option-structure) for some of the arguments or employing the more advanced [variadic options](https://google.github.io/styleguide/go/#variadic-options) technique. The primary consideration for which strategy to choose should be how the function call looks across all expected use cases.
...
1. Create a new package for the validation behavior,[customarily named](https://google.github.io/styleguide/go/#naming-doubles-helper-package) by appending the word `test` to the package name (for example, `chesstest`).
...
For example, `t.Fatal` should be reserved for exceptional cases such as [setup failure](https://google.github.io/styleguide/go/#test-helper-error-handling) as usual:
...
**Warning:** It is not always safe to call `t.Fatal` and similar functions.[More details here](https://google.github.io/styleguide/go/#t-fatal-goroutine).

**Note:** This section discusses [test helpers](https://google.github.io/styleguide/go/decisions#mark-test-helpers) in the sense Go uses the term: functions that perform test setup and cleanup, not common assertion facilities. See the [test functions](https://google.github.io/styleguide/go/#test-functions) section for more discussion.
...
**Warning:** It is not always safe to call `t.Fatal` and similar functions.[More details](https://google.github.io/styleguide/go/#t-fatal-goroutine) here.
...
[Test helpers](https://google.github.io/styleguide/go/#test-functions) usually don’t signal failure from new goroutines, and therefore it is all right for them to use `t.Fatal`. If in doubt, call `t.Error` and return instead.
...
When all calls to the `testing` API are in the [test function](https://google.github.io/styleguide/go/#test-functions), it is usually easy to spot incorrect usage because the `go` keyword is plain to see. Passing `testing.T` arguments around makes tracking such usage harder. Typically, the reason for passing these arguments is to introduce a test helper, and those should not depend on the system under test. Therefore, if a test helper [registers a fatal test failure](https://google.github.io/styleguide/go/#test-helper-error-handling), it can and should do so from the test’s goroutine.
...
[Global state comes in several forms](https://google.github.io/styleguide/go/#globals-forms), and you can use a few [litmus tests to identify when it is safe](https://google.github.io/styleguide/go/#globals-litmus-tests).
...
- Top-level variables irrespective of whether they are exported.
	...
	var Sinks []Sink
	```
	See the [litmus tests](https://google.github.io/styleguide/go/#globals-litmus-tests) to know when these are safe.
- The [service locator pattern](https://en.wikipedia.org/wiki/Service_locator_pattern). See the [first example](https://google.github.io/styleguide/go/#globals). The service locator pattern itself is not problematic, rather the locator being defined as global.
...
[APIs using the patterns above](https://google.github.io/styleguide/go/#globals-forms) are unsafe when:
...
Follow the [litmus tests](https://google.github.io/styleguide/go/#globals-litmus-tests) with these guidelines in such cases:

1. The package must offer clients the ability to create isolated instances of package types as [described above](https://google.github.io/styleguide/go/#globals-forms).
...
4. This package-level API must [document](https://google.github.io/styleguide/go/#documentation-conventions) and enforce its invariants (for example, at which stage in the program’s life it can be called, whether it can be used concurrently). Further, it must provide an API to reset global state to a known-good default (for example, to facilitate testing).
